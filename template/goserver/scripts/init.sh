...
while ! nc -vz 127.0.0.1 8153; do
    ...
    if [ -f "/godata/config/cruise-config.xml" ]; then
        CONTENT='> <security><authConfigs><authConfig id="auth" pluginId="cd.go.authentication.passwordfile"><property><key>PasswordFilePath</key><value>/godata/passwd</value></property></authConfig></authConfigs></security> </server>
